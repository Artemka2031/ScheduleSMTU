...
from Bot.Routers.UserRouters.ScheduleRouter.ScheduleRouters.format_functions import format_schedule, format_dual_week_schedule
...
async def start_messaging(message: Message, state: FSMContext) -> None:
    ...
    sent_message = await message.answer(
        text=f"Текущий тип недели: {hbold(WeekType.get_current_week())}.\n\nВыбранная группа: {hbold(User.get_group_number(message.from_user.id))}\n\nВыберите тип недели:",
